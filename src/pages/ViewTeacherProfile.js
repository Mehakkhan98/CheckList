...
export default class Profile extends Component {
   ...
   askfortution=(user,list)=>{
    ...
    this.state.Comment_data=this.state.Comment_data.concat(list,{"Name":this.state.User,"Password":this.state.Password,"Status":"send"})
      ...
      const newTodo1 = {
        ...
        User_Name:user,
        Student_FriendList:this.state.Comment_data
      
        
        
      };
  
      let response1 = axios.post('http://192.168.8.101:4000/online_tutor_db/updateStudentRequest',newTodo1)
  ...
  Meeting=()=>{
   ...
   getdata= async () => {
     try {
       const value = await AsyncStorage.getItem('Student_Name');
       if (value !== null) {
         // We have data!!
     ...
     try {
       const value1 = await AsyncStorage.getItem('Student_Password');
       if (value1 !== null) {
         this.setState({SPassword:value1})
     ...
     const newTodo = {
       ...
       Student_Password:this.state.SPassword,
  ...
  render()
 {
   console.log("this is my own dtaa",this.state.OwnData)
  ...
  return  this.state.dataSource.map((data,i) => {
    return (
       ...
       <View style={styles.body1}> 
         ...
         <Text style={styles.name}> {data.Student_Name=="Not Define"||null?"address Not Define":data.Student_Name}</Text>
         
    <Paragraph style={styles.info}> {data.Student_Class=="Not Define"||null?"Qualification Not Define":data.Student_Class}</Paragraph>
  <Rating/>
  </View> 
   </View> 
  ...
  <Text   style={{color: "#696969",fontSize:16,marginLeft:10}}  >  {data.Student_Email=="Not Define"||null?"Email Not Define":data.Student_Email} </Text>
  </View>
  <View style={{left:2}}>
  <View  style={styles.description2}>
  
  <Ionicons name="ios-call" size={32} color={Background}/>
  <Text style={{color:Background ,fontSize:22,marginHorizontal:12,marginTop:2,}}>Contact:</Text>
  
  
  
  
  </View>
  {/* <Text  style={{color: "#696969",fontSize:16,marginLeft:10,position:'absolute',marginTop:35}} >  {data.Student_Phone=="Not Define"||null?"Phone Not Define":data.Student_Phone} </Text>
   */}
  ...
  <Text   style={{color: "#696969",fontSize:18,marginLeft:10}}  >  {data.Student_Adress=="Not Define"||null?"address Not Define":data.Student_Adress+" "+data.Teacher_City} </Text>
  ...
  <Text  style={{color: "#696969",fontSize:16,marginLeft:10}}  >  {data.Student_Gender=="Not Define"||null?"Gen Not Define":"hi i am "+data.Student_Gender } </Text>
  </View>
  <View style={{right:15,position:'absolute'}}>
  <View  style={styles.description2}>
  <Ionicons name="ios-bowtie" size={32} color={Background}/>
  <Text style={{color:Background ,fontSize:22,marginHorizontal:5,marginTop:2,}}>Last Grade:</Text>
  
  
  </View>
  <Text   style={{color: "#696969",fontSize:16,marginLeft:10,}} >  {data.Student_Lastgrades=="Not Define"||null?"Grades not defined ":"I want  "+data.Student_Lastgrades+" Student"}</Text>
  ...
  <Text    style={{color: "#696969",fontSize:16,marginLeft:10}} >  {data.Student_Fee_Range=="Not Define"||null?"Range  Not Define":"I can afford "+data.Student_Fee_Range+"/-"}  </Text>
  </View>
 <View style={{right:15,position:'absolute'}}>
  <View  style={styles.description2}>
  ...
  <Text   style={{color: "#696969",fontSize:16,left:60,position:'relative'}} >  {data.Student_Likes=="Not Define"||null?"Likes not defined ":+data.Student_Likes}</Text>
  </View>
  ...
  <Text   style={{color: "#696969",fontSize:16,marginLeft:10,}} >  {data.Student_Nature=="Not Define"||null?"Nature is not defined ":"I am  "+data.Student_Nature+" Student"}</Text>
  ...
  <Text    style={{color: "#696969",fontSize:18,marginLeft:10}} >  {data.Tuttion_Start_Time=="Not Define"||null&&data.Teacher_End_Time=="Not Define"||null?"Tuttion  time is not define ":data.Tuttion_Start_Time+"  to  "+data.Tuttion_End_Time} </Text>
  ...
  <Text  style={{color: "#696969",fontSize:16,marginLeft:10}}>{data.Student_Subjects=="Not Define"||null=="Not Define"||null?"Tuttion  Subject is not define ":"I am expert in  "+data.Student_Subjects} </Text>
  ...
  <Whatsapp  data={data.Student_Phone}/>
  <Sms data={data.Student_Phone}/>
  <Call data={data.Student_Phone}/>
  </View>
  ...
  <TouchableHighlight
      ...
      </View>
      {
      ...
      
       {
          ...
          data.Student_FriendList.map((d)=>{
            console.log("Checkkkkkkkkk",d.Name)
        if(this.state.User===d.Name){
          return(<TouchableOpacity style={{position:'absolute',bottom:3,right:7}}onPress={()=>this.askfortution(data.User_Name,data.Student_FriendList,data.Student_Password)} text="Show More">
